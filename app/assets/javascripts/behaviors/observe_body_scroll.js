		...
		if (window.scrollY < 1) {
			...
			if (loader && (loader.getAttribute("data-loading-preceding-month-link") !== "true")) {
				...
				$.ajax({ url: loader.getAttribute("href") }).done(function(html) {
					...
					var allCalendars = document.querySelectorAll("#reservationskalender .calendar");
					var firstCalendar = allCalendars[0];
					firstCalendar.querySelectorAll("[data-js-enter-reservation-href]").forEach(function(node) {
					...
					var previousMonthUrl = document.querySelector("[data-prev-month-url]").getAttribute("data-prev-month-url");
		...
		if ((window.innerHeight + window.scrollY) > document.body.scrollHeight - 150) {
			...
			if (loader && (loader.getAttribute("data-loading-succeeding-month-link") !== "true")) {
				...
				$.ajax({ url: loader.getAttribute("href") }).done(function(html) {
					...
					var allCalendars = document.querySelectorAll("#reservationskalender .calendar");
					var lastCalendar = allCalendars[allCalendars.length - 1];
					lastCalendar.querySelectorAll("[data-js-enter-reservation-href]").forEach(function(node) {
						node.addEventListener("click", function(e) {
							window.location.href = node.getAttribute("data-js-enter-reservation-href");
						});
					});
					...
					var nextMonthNodes = document.querySelectorAll("[data-next-month-url]");
					var nextMonthUrl = nextMonthNodes[nextMonthNodes.length - 1].getAttribute("data-next-month-url");
