...
class Reservation < ApplicationRecord
  ...
  def duration_in_days
    duration_in_complete_days_evaluated_in_seconds / (24 * 60 * 60)
  end
  ...
  def overlaps_with?(other)
    if other.start == start || other.finish == finish
      true
    elsif other.start == finish || start == other.finish
      false
  ...
  def self.reservations_for_user_in_timeslot(user, time_a, time_b)
    find_reservations_beginning_in_timeslot(time_a, time_b).select { |r| r.user.eql? user }
  end

  def self.reservations_for_user_in_month(user, month)
    find_reservations_beginning_in_month(month).select { |r| r.user.eql? user }
  end

  def self.reservations_for_user_in_year(user, year)
    find_reservations_beginning_in_year(year).select { |r| r.user.eql? user }
  end
  ...
  def self.find_reservations_on_date(date)
    ...
    reservations = find_reservations_in_timeslot(from, to)
  ...
  def is_timeslot_exclusive?
    ...
    unless conflicting_reservations.empty?
      errors.add(:start,
                 'Dieser Zeitabschnitt überlappt mit einer bestehenden Reservation')
    end
  ...
  def is_reservation_not_longer_than_a_week?
    return unless duration > 60 * 60 * 24 * 7

    errors.add(:finish,
               'Anfang und Ende liegen zu weit auseinander. Das Brüschhüsli kann für maximal 7 Tage reserviert werden')
  end
end
