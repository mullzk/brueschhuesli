...
class ReservationTest < ActiveSupport::TestCase
  ...
  test 'Keine negativen oder überlangen Reservationen' do
    assert Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 8, 0o0),
                           finish: DateTime.new(2019, 2, 1, 10, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert_not Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 10, 0o0),
                               finish: DateTime.new(2019, 2, 1, 8, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert_not Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 10, 0o0),
                               finish: DateTime.new(2019, 2, 10, 8, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
  end

  test 'overlapping Reservations' do
    reservation_afternoon = FactoryBot.create(:reservation, user: @user, start: DateTime.new(2019, 2, 1, 14, 0o0),
                                                            finish: DateTime.new(2019, 2, 1, 18, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT)

    assert Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 8, 0o0),
                           finish: DateTime.new(2019, 2, 1, 10, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert_not Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 8, 0o0),
                               finish: DateTime.new(2019, 2, 1, 15, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert_not Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 8, 0o0),
                               finish: DateTime.new(2019, 2, 1, 21, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert_not Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 17, 0o0),
                               finish: DateTime.new(2019, 2, 1, 21, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 2, 1, 19, 0o0),
                           finish: DateTime.new(2019, 2, 1, 21, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
    assert_not Reservation.new(user: @user, is_exclusive: true, start: DateTime.new(2019, 1, 31, 19, 0o0),
                               finish: DateTime.new(2019, 2, 3, 21, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT).valid?
  end

  test 'find all reservations an a day' do
    first_reservation = FactoryBot.create(:reservation, user: @user, start: DateTime.new(2019, 2, 1, 14, 0o0),
                                                        finish: DateTime.new(2019, 2, 2, 18, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT)
    second_reservation = FactoryBot.create(:reservation, user: @user, start: DateTime.new(2019, 2, 2, 20, 0o0),
                                                         finish: DateTime.new(2019, 2, 3, 18, 0o0), type_of_reservation: Reservation::KURZAUFENTHALT)
  ...
  test 'find reservations that span multiple days' do
    res =  FactoryBot.create(:reservation, user: @user, start: DateTime.new(2010, 2, 4, 8, 15),
                                           finish: DateTime.new(2010, 2, 7, 10, 0), type_of_reservation: Reservation::KURZAUFENTHALT)

    # Interesting Timeslot is bigger or equal than reservation
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 7),
                                                     DateTime.new(2010, 2, 7, 23)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 8),
                                                     DateTime.new(2010, 2, 7, 23)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 8, 14),
                                                     DateTime.new(2010, 2, 7, 23)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 8, 15),
                                                     DateTime.new(2010, 2, 7, 23)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 7),
                                                     DateTime.new(2010, 2, 7, 11)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 7),
                                                     DateTime.new(2010, 2, 7, 10)).include?(res)

    # Interesting Timeslot collides on one side with reservation
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 8, 15),
                                                     DateTime.new(2010, 2, 7, 9)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 9),
                                                     DateTime.new(2010, 2, 7, 23)).include?(res)

    # Interesting Timeslot is inside a reservation
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 9),
                                                     DateTime.new(2010, 2, 7, 9)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 10),
                                                     DateTime.new(2010, 2, 4, 11)).include?(res)

    # The order is of no importance
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 11),
                                                     DateTime.new(2010, 2, 4, 10)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 6, 11),
                                                     DateTime.new(2010, 2, 4, 10)).include?(res)

    # Timeslot before
    assert_not Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 7),
                                                         DateTime.new(2010, 2, 4, 8)).include?(res)
    assert_not Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 7),
                                                         DateTime.new(2010, 2, 4, 8, 15)).include?(res)
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 4, 7),
                                                     DateTime.new(2010, 2, 4, 9)).include?(res)

    # Timeslot after
    assert Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 7, 9),
                                                     DateTime.new(2010, 2, 7, 12)).include?(res)
    assert_not Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 7, 10),
                                                         DateTime.new(2010, 2, 7, 12)).include?(res)
    assert_not Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 7, 11),
                                                         DateTime.new(2010, 2, 7, 12)).include?(res)
    assert_not Reservation.find_reservations_in_timeslot(DateTime.new(2010, 2, 8, 8),
                                                         DateTime.new(2010, 2, 9, 9)).include?(res)
  end

  test 'find reservations in timeslot on one day' do
    r1 =  FactoryBot.create(:reservation, user: @user, start: DateTime.new(2010, 2, 4, 8, 15),
                                          finish: DateTime.new(2010, 2, 7, 10, 0), type_of_reservation: Reservation::KURZAUFENTHALT)
    r2 =  FactoryBot.create(:reservation, user: @user, start: DateTime.new(2010, 2, 8, 8, 15),
                                          finish: DateTime.new(2010, 2, 8, 10, 0), type_of_reservation: Reservation::KURZAUFENTHALT)
    r3 =  FactoryBot.create(:reservation, user: @user, start: DateTime.new(2010, 2, 8, 13, 0),
                                          finish: DateTime.new(2010, 2, 8, 15, 0), type_of_reservation: Reservation::KURZAUFENTHALT)
  ...
  test 'long KURZAUFENTHALT should be interpreted as FERIENAUFENTHALT' do
    ...
    r1 = Reservation.new(start: DateTime.new(2010, 6, 1, 12), finish: DateTime.new(2010, 6, 3, 12),
                         type_of_reservation: Reservation::KURZAUFENTHALT)
    assert_equal r1.type_of_reservation, Reservation::KURZAUFENTHALT
    r1 = Reservation.new(start: DateTime.new(2010, 6, 1, 12), finish: DateTime.new(2010, 6, 3, 13),
                         type_of_reservation: Reservation::KURZAUFENTHALT)
    assert_equal r1.type_of_reservation, Reservation::FERIENAUFENTHALT
    r1 = Reservation.new(start: DateTime.new(2010, 6, 1, 12), finish: DateTime.new(2010, 6, 2, 11),
                         type_of_reservation: Reservation::FERIENAUFENTHALT)
    assert_equal r1.type_of_reservation, Reservation::KURZAUFENTHALT
    r1 = Reservation.new(start: DateTime.new(2010, 6, 1, 12), finish: DateTime.new(2010, 6, 3, 12),
                         type_of_reservation: Reservation::FERIENAUFENTHALT)
    assert_equal r1.type_of_reservation, Reservation::FERIENAUFENTHALT
    r1 = Reservation.new(start: DateTime.new(2010, 6, 1, 12), finish: DateTime.new(2010, 6, 3, 11),
                         type_of_reservation: Reservation::FERIENAUFENTHALT)
    assert_equal r1.type_of_reservation, Reservation::KURZAUFENTHALT
  end

  test 'Reservations should be ordered by Start date' do
    r1 = FactoryBot.create(:kurzaufenthalt_for_testuser, start: DateTime.new(2010, 6, 5, 12),
                                                         finish: DateTime.new(2010, 6, 6, 12))
    r2 = FactoryBot.create(:kurzaufenthalt_for_testuser, start: DateTime.new(2010, 6, 1, 12),
                                                         finish: DateTime.new(2010, 6, 3, 11))
    r3 = FactoryBot.create(:kurzaufenthalt_for_testuser, start: DateTime.new(2010, 6, 3, 19),
                                                         finish: DateTime.new(2010, 6, 4, 20))
    r4 = FactoryBot.create(:kurzaufenthalt_for_testuser, start: DateTime.new(2010, 6, 3, 12),
                                                         finish: DateTime.new(2010, 6, 3, 18))
  ...
  test 'reservations beginning on timeslot for accounting' do
    ...
    res_kaspar1 = Reservation.create(start: DateTime.new(2010, 2, 1, 16), finish: DateTime.new(2010, 2, 1, 18),
                                     user: kaspar)
    res_ruth1   = Reservation.create(start: DateTime.new(2010, 2, 3, 8, 15), finish: DateTime.new(2010, 2, 3, 10),
                                     user: ruth)
    res_kaspar2 = Reservation.create(start: DateTime.new(2010, 2, 4, 8, 15), finish: DateTime.new(2010, 2, 7, 10),
                                     user: kaspar)
    res_kaspar3 = Reservation.create(start: DateTime.new(2010, 2, 8, 8, 15), finish: DateTime.new(2010, 2, 8, 10),
                                     user: kaspar)
    res_ruth2   = Reservation.create(start: DateTime.new(2010, 2, 8, 13), finish: DateTime.new(2010, 2, 8, 15),
                                     user: ruth)
    res_stefan  = Reservation.create(start: DateTime.new(2010, 2, 27, 13), finish: DateTime.new(2010, 3, 3, 15),
                                     user: stefan)
