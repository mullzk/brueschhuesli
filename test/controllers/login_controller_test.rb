...
class LoginControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'update user' do
    ...
    post '/login/update_user',
         params: { id: user.id, user: { name: 'NewName', email: 'NewMail', password: 'NewPassword' } }
    assert_redirected_to controller: 'login', action: 'list_users'
  ...
  test 'change password' do
    ...
    post '/login/change_password',
         params: { id: user.id, old_password: 'abc', user: { password: '456', password_confirmation: '456' } }
    assert_equal 'Altes Passwort ist ungültig', flash[:notice]
    assert_response :success
    post '/login/change_password',
         params: { id: user.id, old_password: '123', user: { password: '456', password_confirmation: '798' } }
    assert_response :success
    assert User.authenticate(user.name, '123')
    post '/login/change_password',
         params: { id: user.id, old_password: '123', user: { password: '456', password_confirmation: '456' } }
